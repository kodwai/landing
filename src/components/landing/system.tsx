...
export function HeroVideo({ tone = "light" }: { tone?: "light" | "dark" }) {
  ...
  // Callback ref that forces the demo SILENT from the very first frame.
  // React doesn't reliably reflect the `muted` PROP to the DOM, and paired with
  // the `autoPlay` attribute that race could let the video start WITH SOUND
  // before the effect below runs (scaring visitors on load). Setting
  // `defaultMuted` writes the actual `muted` CONTENT attribute the browser's
  // autoplay policy reads, so it stays muted on hard loads and soft navigations.
  const attach = useCallback((node: HTMLVideoElement | null) => {
    if (node) { node.defaultMuted = true; node.muted = true; }
    ref.current = node;
  }, []);
