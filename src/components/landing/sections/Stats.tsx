...
/* ══════════════════════════════════════════════════════════════════════════
   STATS BAND  ·  // 07 · the proof
   ───────────────────────────────────────────────────────────────────────────
   A calm, precise, industry-standard editorial stat strip. Four proof points
   set in a single horizontal row, separated by thin vertical hairline rules.
   Each cell: a small mono kicker on top, a large Fraunces serif numeral on a
   shared baseline, then a one-to-two line grotesk caption. Everything is
   left-aligned within its cell and perfectly aligned across all four.

   The previous version (loose 2x2 grid with hand-drawn doodle scribbles) read
   messy and misaligned; this is its opposite. No doodles, no scribbles.

   Layout: CSS grid, 4 cols desktop -> 2 cols tablet -> 1 col mobile, with
   consistent gaps and dividers drawn as cell borders so they always line up.
   Numerals count up via the system Counter; content renders with no JS and the
   count-up self-gates under prefers-reduced-motion (built into Counter).
   ══════════════════════════════════════════════════════════════════════════ */
...
const ITEMS = STATS as readonly StatItem[];

/* Mono kicker per stat, index-aligned to STATS. */
const KICKERS = ["the category", "the setup", "the price", "the score"] as const;

export default function Stats() {
  const K = "k-stats-"; // unique scoped-style prefix (collision-proof)

  return (
    <Band tone="paper2" id="stats">
      <ScopedStyle k={K} />

      <Inner>
        <Marker index="07" label="the proof" />

        {/* ── header: serif framing line + a quiet mono sub-note ── */}
        <div
          style={{
            display: "flex",
            flexWrap: "wrap",
        ...
        >
          <Serif as="h2" size="h2" className="k-reveal" style={{ maxWidth: "17ch" }}>
            Numbers we are happy to <Accent>stand behind</Accent>.
          </Serif>
          ...
          >
            No vanity metrics. Just what the platform is, what it costs you, and how
            honestly it measures the way you actually work.
          </p>
        </div>
        ...
        <div className={`k-reveal ${K}foot`}>
          <span className={`${K}footTag`}>measured, not marketed</span>
          <span className={`${K}footRule`} aria-hidden />
          <span className={`${K}footNote`}>every signal cites its own evidence</span>
        </div>
      </Inner>
    </Band>
  );
